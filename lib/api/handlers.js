...
const { paramsForYear } = require('../../tools/overrides/statutory-params'); // the SAME source the math uses
...
async function metaHandler(db) {
  ...
  return { status: 200, body: { quarter: m.puf_quarter, ingestedAt: m.finished_at, downloadDate: m.download_date, scope: m.scope, rowCounts: m.row_counts, planYear, oopCapAnnual, source: 'CMS Prescription Drug Plan Formulary, Pharmacy Network, and Pricing PUF' } };
}
