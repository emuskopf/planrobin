...
const ANNUAL_FORMULA = 'Estimated annual cost = (monthly premium × 12) + (each covered drug’s 30-day retail copay × 12), initial-coverage phase. Coinsurance drugs can’t be dollar-totaled without knowing your dose, so they’re listed separately with the plan’s per-unit negotiated price (where the data has it) rather than folded into this number.';
...
async function resultsHandler(db, body) {
  ...
  for (const p of county_.plans) {
    ...
    let notCovered = 0;
    const notCoveredRx = [];    // drugs not on this plan's formulary (loud, listed by name, never $0)
    const coinsuranceRx = [];   // coinsurance drugs we can't dollar-total (shown per-unit, honest)
    for (const rxcui of rx) {
      ...
      drugs[rxcui] = {
        ...
        // Pricing file: the plan's negotiated per-unit price + how we're basing the cost.
        costBasis: d.costBasis, negotiatedPrice: d.negotiatedPrice,
      };
      if (d.costBasis === 'coinsurance_per_unit' || d.costBasis === 'coinsurance_no_price') coinsuranceRx.push(rxcui);
    }
    ...
    // Itemized, honest breakdown (Half 2). The headline total is the SUM of what we can honestly
    // total — premium + flat-copay (incl. statutorily-capped insulin/vaccine) drugs. Coinsurance
    // drugs can't be dollar-totaled without a dose, so they're listed separately (with the plan's
    // real per-unit price where the file has it), never folded in as a fake number. Not-covered
    // drugs are listed loudly and never contribute $0 to the total.
    const breakdown = {
      premiumAnnual: proj.annualPremium,
      copayAnnual: proj.annualDrugOOP,               // flat copays (+ insulin/vaccine overrides)
      total: proj.annualTotal,                        // == headline == premium + copayAnnual
      coinsuranceRxcuis: coinsuranceRx,               // shown per-unit / un-annualized
      notCoveredRxcuis: notCoveredRx,                 // loud, listed by name, excluded from total
      hasUnpriceable: coinsuranceRx.some((r) => drugs[r] && drugs[r].costBasis === 'coinsurance_no_price'),
