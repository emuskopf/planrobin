...
const { searchProducts, nameByRxcui } = require('../rxnorm');
...
// ---- POST /api/rxnorm/resolve  { rxcuis:[] } -----------------------------
// rxcui -> name + brand/generic, for the restore-code entry's per-line confirmation. Batch (the entry
// resolves all typed lines at once). Best-effort per rxcui: an unknown/upstream-down entry returns
// name:null and the client labels it by rxcui — never blocks the restore. NO LLM.
async function rxnormResolveHandler(body, deps = {}) {
  const fetchImpl = deps.fetch || fetch;
  const db = deps.db;
  const rxcuis = ((body && body.rxcuis) || []).map((r) => String(r).replace(/\D+/g, '')).filter(Boolean).slice(0, 12);
  if (!rxcuis.length) return { status: 200, body: { results: [] } };
  const resolved = await Promise.all(rxcuis.map((rx) => nameByRxcui(rx, fetchImpl).catch(() => null)));
  const results = rxcuis.map((rx, i) => resolved[i] || { rxcui: rx, name: null, kind: 'generic' });
  // Flag which are on a Missouri formulary (same annotation as search), best-effort.
  if (db) {
    try {
      ...
      for (const r of results) r.onFormulary = covered.has(r.rxcui);
    } catch (_) { /* leave unannotated */ }
  }
  return { status: 200, body: { results }, headers: { 'Cache-Control': 'public, max-age=604800, stale-while-revalidate=604800' } };
}
...
module.exports = { countiesHandler, zipHandler, metaHandler, rxnormSearchHandler, rxnormResolveHandler, resultsHandler, ANNUAL_FORMULA };
