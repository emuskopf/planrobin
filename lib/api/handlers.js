...
const { projectAnnual } = require('../../tools/overrides');
...
async function resultsHandler(db, body) {
  ...
  for (const p of county_.plans) {
    ...
    const drugs = {};
    const drugModels = [];           // fed to the statutory annual projection (rules 4-5)
    ...
    for (const rxcui of rx) {
      ...
      const ov = d.overrides || { effectivePerFill: {}, applied: [], deductibleApplies: false };
      ...
      // 30-day model for the annual projection uses the override-applied per-fill cost.
      drugModels.push({ rxcui, perFill: ov.effectivePerFill, deductibleApplies: ov.deductibleApplies });
      drugs[rxcui] = {
    ...
    // Rules 4-5: annual out-of-pocket with correct fills/year and the $2,100 cap accrual.
    const proj = projectAnnual({ premium: p.premium, deductible: p.deductible, planYear: dc.planYear || 2026, drugs: drugModels, daysSupply: 1 });
